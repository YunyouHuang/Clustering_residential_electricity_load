...
public class InitialNet_In  {
    ...
    */
    ...
    public static void initial3V(String inputPath,JavaSparkContext sc,String savePath,
                                 HashMap<Integer,HashMap<Integer, Float>> neigbhor,String in_Path,String save_Path,int partNum) throws IOException {
        ...
        sc.textFile(inputPath+"/"+in_Path).repartition(partNum).map(new trimForEdge()).map(new calEdgeDist()).saveAsTextFile(savePath+"/"+save_Path);
    ...
    public static void main(String[] args) throws IOException {
        ...
        SparkConf conf=new SparkConf().setAppName("InitialNet_In");//.setMaster(master).setJars(jarPathArray);
        JavaSparkContext sc = new JavaSparkContext(conf);

        HashMap<Integer,HashMap<Integer, Float>> neigbhor=loadingEdge(inputPath+"/result");


        //initial2V(lines,sc,outputPath,neigbhor);

        for (int i = _start; i <10 ; i++) {
            String inPath="tmp_result/"+i;
            String savePath="edge/"+i;
            //initial3V(sc,outputPath,neigbhor,exInPath,exSavePath,partNum);
            initial3V(inputPath,sc,outputPath,neigbhor,inPath,savePath,partNum);
        }
