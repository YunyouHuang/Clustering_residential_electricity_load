...
public class Sil {
    ...
    public Sil(String dataPath,int win){
            ...
            for (int i = 0; i < value.size(); i++) {
                ...
                if (value.size()>1){
                    for (int j = 0; j < value.size(); j++) {
                        if (i!=j){
                            double tmpDist=distanceMatrix[idMap.get(x.get_1())][idMap.get(value.get(j).get_1())];
                            ax+=tmpDist;
                        }
                    }
                    ax=ax/(value.size()-1);
                ...
                for (Map.Entry<Integer, ArrayList<Tulpe2SA>> entry_y : data.entrySet()) {
                    ...
                    if (key!=key_2){
                        ...
                        for (int j = 0; j < value_y.size(); j++) {
                            //System.out.println("index "+idMap.get(x.get_1())+"-"+idMap.get(value.get(j).get_1()));
                            double tmpDist=distanceMatrix[idMap.get(x.get_1())][idMap.get(value_y.get(j).get_1())];
                            c_arg+=tmpDist;
                        }
                        c_arg=c_arg/value_y.size();
                        if (c_arg<bx){
                            bx=c_arg;
                        }
                    }
                ...
                sil+=(bx-ax)/Math.max(ax,bx);

            }
        }

        return centers.size()+","+rare+","+(sil/dataSetNum)+"\n";
    }
    ...
    public static void main(String[] args) {
        ...
        Sil di=new Sil(dataSetPath,win);
