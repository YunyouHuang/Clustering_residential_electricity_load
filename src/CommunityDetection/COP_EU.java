...
public class COP_EU {
    ...
    public String evaluation(String dataPath,String centerPath){
        ...
        for (Map.Entry<Integer, ArrayList<Tulpe2SA>> entry : data.entrySet()) {
            ...
            if (value.size()>1){
                for (int i = 0; i < value.size(); i++) {
                    ...
                    intra+=dist.getEuclideanDistance(tmpCenter,value.get(i).get_2());
                }
        ...
        //System.out.print(centers.size()+","+rare+","+(cop/dataSetNum));
    ...
    public static void main(String[] args) {
       ...
       // String[] rateArray=rate_2.split(",");
        ...
        COP_EU di=new COP_EU(dataSetPath);

        //for (int i = 0; i < rateArray.length; i++) {
        //    String w=di.evaluation(data_cd_Path+"r_3_075_1_1_"+rateArray[i],data_cd_Path+"centers/r_3_075_1_1_"+rateArray[i]+"_3_centers/centers.txt");
            //System.out.println(dataPath);
        //    System.out.print(w);
         //   FileWrite.WriteTxt(w,savePath_cd);
       // }
        for (int i = 0; i < 61; i++) {
            String w=di.evaluation(data_path+"CFSFDP/CFSFDP_"+i+"/data",data_path+"CFSFDP/CFSFDP_"+i+"/centers/centers.txt");
            System.out.println(data_path+"CFSFDP");
            System.out.print(w);
            FileWrite.WriteTxt(w,data_path+"CFSFDP_0409_cop.txt");
        }
        /*
        for (int i = 0; i < 61; i++) {
            String w=di.evaluation(data_path+"AgglomerativeClustering/AgglomerativeClustering_"+i+"/data",data_path+"AgglomerativeClustering/AgglomerativeClustering_"+i+"/centers/centers.txt");
            System.out.println(data_path+"AgglomerativeClustering");
            System.out.print(w);
            FileWrite.WriteTxt(w,data_path+"AgglomerativeClustering_0409_cop.txt");
        }

        for (int i = 0; i < 61; i++) {
            String w=di.evaluation(data_path+"GaussianMixture/GaussianMixture_"+i+"/data",data_path+"GaussianMixture/GaussianMixture_"+i+"/centers/centers.txt");
            System.out.println(data_path+"GaussianMixture");
            System.out.print(w);
            FileWrite.WriteTxt(w,data_path+"GaussianMixture_0409_cop.txt");
        }

        for (int i = 0; i < 61; i++) {
            String w=di.evaluation(data_path+"KMeans/KMeans_"+i+"/data",data_path+"KMeans/KMeans_"+i+"/centers/centers.txt");
            System.out.println(data_path+"KMeans");
            System.out.print(w);
            FileWrite.WriteTxt(w,data_path+"KMeans_0409_cop.txt");
        }

        for (int i = 0; i < 61; i++) {
            String w=di.evaluation(data_path+"SpectralClustering/SpectralClustering_"+i+"/data",data_path+"SpectralClustering/SpectralClustering_"+i+"/centers/centers.txt");
            System.out.println(data_path+"SpectralClustering");
            System.out.print(w);
            FileWrite.WriteTxt(w,data_path+"SpectralClustering_0409_cop.txt");
        }
        */
/*
