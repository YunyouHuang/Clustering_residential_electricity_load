...
public class S_Dbw {
    ...
    //计算方差
    public static List<Tuple2<Integer,Double>> getVariance(JavaPairRDD<Integer,ElectricProfile> dataSet, HashMap<Integer,ArrayList<Double>> averSquence, final int win){
        final HashMap<Integer,ArrayList<Double>> _averSquence=averSquence;
        final int dtw_win=win;

        class getVariance implements PairFunction<Tuple2<Integer,ElectricProfile>,Integer,Tuple2<Double,Integer>>{
            @Override
            public Tuple2<Integer, Tuple2<Double,Integer>> call(Tuple2<Integer, ElectricProfile> integerElectricProfileTuple2) throws Exception {
                Distance dist=new Distance();
                return new Tuple2<Integer, Tuple2<Double,Integer>>(integerElectricProfileTuple2._1,new Tuple2<Double,Integer>(dist.getDTWVarianceForDouble(_averSquence.get(integerElectricProfileTuple2._1),integerElectricProfileTuple2._2.getPoints(),dtw_win),1));
            }
        }

        class sumForVaiance implements Function2<Tuple2<Double,Integer>,Tuple2<Double,Integer>,Tuple2<Double,Integer>>{
            @Override
            public Tuple2<Double, Integer> call(Tuple2<Double, Integer> doubleIntegerTuple2, Tuple2<Double, Integer> doubleIntegerTuple22) throws Exception {
                return new Tuple2<Double, Integer>(doubleIntegerTuple2._1+doubleIntegerTuple22._1,doubleIntegerTuple2._2+doubleIntegerTuple22._2);
            }
        }

        class getValue implements PairFunction<Tuple2<Integer,Tuple2<Double,Integer>>,Integer,Double>{
            @Override
        ...
        return dataSet.mapToPair(new getVariance()).reduceByKey(new sumForVaiance()).mapToPair(new getValue()).collect();
    }
    ...
    //获取所有类的平均方差
    public static double getMeansVar(List<Tuple2<Integer,Double>> var,int stdFlag,double stdRate){

        double retValue=0;
        for (int i = 0; i < var.size(); i++) {
            retValue+=var.get(i)._2;
        }
    ...
    public static Tuple2<Double,Double> getS_Dbw(double dataSetVar,HashMap<Integer,ArrayList<Double>> averSquence,int dtw_win, JavaPairRDD<Integer,ElectricProfile> dataSet, JavaSparkContext sc ,int stdFlag,double stdRate){
        dataSet.cache();
        DataOperation doper=new DataOperation();

        List<Tuple2<Integer,Double>> centerVariance=getVariance(dataSet,averSquence,dtw_win);

        final double stdev=getMeansVar(centerVariance,stdFlag,stdRate);
        final HashMap<Integer,ArrayList<Double>> _averSquence=averSquence;
        ...
        for (int i = 0; i < centerVariance.size(); i++) {
            varCenters+=centerVariance.get(i)._2;
        }
    ...
    public static double getDataSetVar(JavaSparkContext sc,String dataSet_path,int win,String varSavestr) throws IOException {
        ...
        if (runVar){
            ...
            dataSetVaiance=getVariance(dataSet_with_mark,dataSetCenter,win).get(0)._2;
    ...
    public static void main(String[] args) throws IOException {
        ...
        int stdFlag=Integer.parseInt(args[5]);
        double stdRate=Double.parseDouble(args[6]);


        SparkConf conf=new SparkConf().setAppName("S_Dbw_0716");//.setMaster(master).setJars(jarPathArray);
        ...
        double dataSetVaiance=getDataSetVar(sc,dataSet_path,win,"hdfs://172.16.48.108:9000/Community/norBySum_filter/dataSetVar_0716.txt");
        HashMap<Integer,ArrayList<Double>> centers=getCenters(centers_path);

        Tuple2<Double,Double> index=getS_Dbw(dataSetVaiance,centers,win,dataSet_cluster,sc,stdFlag,stdRate);
