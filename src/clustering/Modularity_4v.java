 ...
 * @author pjmcswee
 */
public class Modularity_4v implements Statistics, LongTask {
    ...
    public HashMap<Integer,Integer> getCommunity(){
        //Map<Integer, Integer> sizeDist = new HashMap<>();
        HashMap<Integer,Integer> retValueHashMap=new HashMap<Integer,Integer>();
        for (Node n : structure.graph.getNodes()) {
            Integer v = (Integer) n.getAttribute(MODULARITY_CLASS);
            //System.out.println("Node: "+n.getId()+"-"+n.getLabel()+"   CommID: "+n.getAttribute(MODULARITY_CLASS));
            retValueHashMap.put(Integer.parseInt(n.getLabel()), v);
    ...
    public CommunityStructure clone_structure(CommunityStructure source_structure, int nodeNumber){
        CommunityStructure target_structure=new CommunityStructure(nodeNumber);
        
        target_structure.graph=source_structure.graph;
        target_structure.map=source_structure.map;
        target_structure.topology=source_structure.topology;
        target_structure.invMap=source_structure.invMap;
        target_structure.graphWeightSum=source_structure.graphWeightSum;
        
        double[] target_weights=new double[nodeNumber];
        for (int i = 0; i < target_weights.length; i++) {
            target_weights[i]=source_structure.weights[i]; 
        }
        target_structure.weights=target_weights;
        
        HashMap<Integer,Community> new_ALL_Communities=new HashMap<Integer,Community>();
        
        HashMap<Community, Float>[] s_nodeConnectionsWeight=source_structure.nodeConnectionsWeight;
        HashMap<Community, Integer>[] s_nodeConnectionsCount=source_structure.nodeConnectionsCount;
        Community[] s_nodeCommunities=source_structure.nodeCommunities;
        List<Community> s_communities=source_structure.communities;
        
        HashMap<Community, Float>[] target_nodeConnectionsWeight=new HashMap[nodeNumber];
        HashMap<Community, Integer>[] target_nodeConnectionsCount=new HashMap[nodeNumber];
        
        Community[] target_nodeCommunities=new Community[nodeNumber];
        List<Community> target_communities=new ArrayList<>();
        
        for (int i = 0; i < nodeNumber; i++) {
            Community s_nodeCommunity = s_nodeCommunities[i];
            Community new_nodeCommunity=new Community(target_structure);
                        
            List<Integer> new_nodes=new ArrayList<>();
            for (int j = 0; j < s_nodeCommunity.nodes.size(); j++) {
                new_nodes.add(s_nodeCommunity.nodes.get(j));
            }
            new_nodeCommunity.nodes=new_nodes;
            int index=s_nodeCommunity.get_first_node_index();
            new_ALL_Communities.put(index, new_nodeCommunity);
            target_nodeCommunities[i]=new_nodeCommunity;
            target_communities.add(new_nodeCommunity);
        }
        
        for (int i = 0; i < nodeNumber; i++) {
            Community s_nodeCommunity = s_nodeCommunities[i];
            Community new_nodeCommunity=target_nodeCommunities[i];
            
            HashMap<Community, Float> s_c_connectionsWeight = s_nodeCommunity.connectionsWeight;
            HashMap<Community, Integer> s_c_connectionsCount = s_nodeCommunity.connectionsCount;
            
            HashMap<Community, Float> new_c_connectionsWeight = new HashMap<>();
            HashMap<Community, Integer> new_c_connectionsCount = new HashMap<>();
            
            for (Map.Entry<Community, Integer> entry : s_c_connectionsCount.entrySet()) {
                Community key = entry.getKey();
                Integer value = entry.getValue();
                
                int index=key.get_first_node_index();
                new_c_connectionsCount.put(new_ALL_Communities.get(index), value);
                new_c_connectionsWeight.put(new_ALL_Communities.get(index), s_c_connectionsWeight.get(key));
            }
            new_nodeCommunity.connectionsCount=new_c_connectionsCount;
            new_nodeCommunity.connectionsWeight=new_c_connectionsWeight;
        }
                
        target_structure.nodeCommunities=target_nodeCommunities;
        target_structure.communities=target_communities;
        
        for (int i = 0; i < s_nodeConnectionsCount.length; i++) {
            HashMap<Community, Float> s_nw=s_nodeConnectionsWeight[i];
            HashMap<Community, Integer> s_nc=s_nodeConnectionsCount[i];
            
            HashMap<Community, Float> t_nw=new HashMap<Community,Float>();
            HashMap<Community, Integer> t_nc=new HashMap<Community,Integer>();
            
            for (Map.Entry<Community, Integer> entry : s_nc.entrySet()) {
                Community key = entry.getKey();
                Integer value = entry.getValue();
                
                int index =key.get_first_node_index();
                t_nc.put(new_ALL_Communities.get(index), value);
                t_nw.put(new_ALL_Communities.get(index), s_nw.get(key));
            }
            target_nodeConnectionsWeight[i]=t_nw;
            target_nodeConnectionsCount[i]=t_nc;
            //Community t_nodeCommunity = t_nodeCommunities[i];
            
        }
        
        target_structure.nodeConnectionsWeight=target_nodeConnectionsWeight;
        target_structure.nodeConnectionsCount=target_nodeConnectionsCount;
    ...
    class CommunityStructure {
        ...
        HashMap<Node, Integer> map;//不变
        Community[] nodeCommunities;
        Graph graph;//不变
        double[] weights;
        double graphWeightSum;
        List<ModEdge>[] topology;//不变
        List<Community> communities;
        int N;
        HashMap<Integer, Community> invMap; //不变

        CommunityStructure(int nodeNumber){
            //this.graph = graph;
            N = nodeNumber;
            //invMap = new HashMap<>();
        ...
        CommunityStructure(Graph graph) {
            ...
            for (Node node : nodesIterable) {
                ...
                for (Node neighbor : uniqueNeighbors) {
                    ...
                    Modularity_4v.ModEdge me = new ModEdge(node_index, neighbor_index, weight);
        ...
        private void addNodeTo(int node, Community to) {
            ...
            for (ModEdge e : topology[node]) {
                ...
                ///////////////////
                Modularity_4v.Community adjCom = nodeCommunities[neighbor];
        ...
        private void zoomOut() {
            ...
            for (int i = 0; i < communities.size(); i++) {//Community com : mCommunities) {
                ...
                for (Modularity_4v.Community adjCom : iter) {
    ...
    class Community {
        ...
        public int get_first_node_index(){
            if (nodes.size()>0) {
                return nodes.get(0);
            }else{
                System.out.println("Get ket Error !");
                return -1;
            }
        }
      
        public Community(Modularity_4v.Community com) {
    ...
    public void execute_4v(Graph graph,CommunityStructure structure_4v) {
        isCanceled = false;
        System.out.println("Nodes: " + graph.getNodeCount());
        System.out.println("Edges: " + graph.getEdgeCount());
        ...
        graph.readLock();
        try {
            //structure = new Modularity_4v.CommunityStructure(graph);
            structure=structure_4v;
    ...
    protected HashMap<String, Double> computeModularity(Graph graph, CommunityStructure theStructure, int[] comStructure,
        ...
        int repeat_out=0;
        while (someChange) {
            System.out.println("Processing net layer: #"+repeat_out);
            someChange = false;
            ...
            while (localChange) {
                ...
                for (int i = start; step < theStructure.N; i = (i + 1) % theStructure.N) {
                    step++;
                    //if (step%20000==0) {
                    //    System.out.println("Processing node index: #"+repeat_out+"  *"+repeat_in+"  order-"+step);
                    //}
